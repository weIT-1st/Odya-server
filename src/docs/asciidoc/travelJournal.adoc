...
operation::travel-journal-controller-test/get-travel-journals-success[snippets='http-request,request-headers,path-parameters,http-response,response-fields']

=== *2-2 실패 - 비공개 여행 일지이지만, 작성자가 요청하지 않은 경우*

operation::travel-journal-controller-test/get-travel-journals-fail-not-writer[snippets='http-request,request-headers,path-parameters,http-response']

=== *2-3 실패 - 친구 공개 여행 일지이지만, 친구가 아닌 사용자가 요청한 경우"*

operation::travel-journal-controller-test/get-travel-journals-fail-not-friend[snippets='http-request,request-headers,path-parameters,http-response']

=== *2-4 실패 - 유효하지 않은 토큰일 경우"*

operation::travel-journal-controller-test/get-travel-journals-fail-invalid-token[snippets='http-request,request-headers,path-parameters,http-response']

[[나의여행일지목록조회-API]]
== *3. 나의 여행 일지 목록 조회 API*
...
=== *6-5 실패 - 여행 일지 기간과 여행 일지 콘텐츠 개수가 다른 경우*
...
=== *6-6 실패 - 여행 일지 콘텐츠의 날짜와 여행 시작일, 종료일 사이에 없는 경우*
...
=== *6-7 실패 - 여행 친구 수가 제한 친구 수보다 많은 경우*
...
=== *7-2 실패 - 여행 일지 콘텐츠 아이디에 맞는 여행 일지 콘텐츠가 없는 경우*
...
=== *7-4 실패 - 여행 일지 콘텐츠의 이미지 개수가 제한 개수보다 많은 경우*
...
=== *7-5 실패 - 추가할 여행 일지 콘텐츠의 이름과 이미지 파일의 이름이 다른 경우*
...
=== *7-6 실패 - 여행 일지 콘텐츠 여행 일자가 여행 시작일과 종료일 사이에 없는 경우*
...
=== *9-3 실패 - 여행 일지 콘텐츠 아이디와 일치하는 여행 일지 콘텐츠가 없는 경우*
