...
class PlaceSearchHistoryServiceTest : DescribeSpec(
    {
        ...
        describe("saveSearchHistory 메소드") {
            context("유효한 데이터가 전달되면") {
                ...
                it("리뷰를 생성한다.") {
                    shouldNotThrowAny { sut.saveSearchHistory(TEST_PLACE_SEARCH_TERM, TEST_USER_ID) }
                }
            }
            ...
            context("가입되어 있지 않은 USERID가 주어지는 경우") {
                ...
                it("[NoSuchElementException] 예외가 발생한다") {
                    shouldThrow<NoSuchElementException> { sut.saveSearchHistory(TEST_PLACE_SEARCH_TERM, TEST_NOT_EXIST_USER_ID) }
                }
            }
        }
        ...
        describe("getOverallRanking 메소드") {
            context("유효한 요청이 들어오면") {
                ...
                it("검색어 전체 순위 출력") {
                    shouldNotThrowAny { sut.getOverallRanking() }
                }
            }
        }
        ...
        describe("getAgeRangeRanking 메소드") {
            context("로그인한 유저와 연령대를 전달되면") {
                ...
                it("해당 연령대 순위 출력") {
                    shouldNotThrowAny { sut.getAgeRangeRanking(TEST_USER_ID, TEST_AGE_RANGE) }
                }
            }
            ...
            context("로그인한 유저만 전달되면") {
                ...
                every { placeSearchHistoryRepository.getRecentTop10Keywords(TEST_AGE_RANGE) } returns createListSearchTerm()
                it("유저 연령대 순위 출력") {
                    shouldNotThrowAny { sut.getAgeRangeRanking(TEST_USER_ID, null) }
                }
            }
            ...
            context("가입되어 있지 않은 USERID가 전달되면") {
                every { userRepository.getByUserId(TEST_NOT_EXIST_USER_ID) } throws NoSuchElementException()
                it("[NoSuchElementException] 예외가 발생한다") {
                    shouldThrow<NoSuchElementException> { sut.getAgeRangeRanking(TEST_NOT_EXIST_USER_ID, null) }
                }
            }
        }
    },
