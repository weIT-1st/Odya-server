...
import kr.weit.odya.support.createSlicePlaceReview
...
class PlaceReviewServiceTest : DescribeSpec(
    {
        ...
        describe("getByPlaceReviewList 메소드") {
            context("유효한 placeId가 전달되면") {
                every { placeReviewRepository.getByPlaceIdInitial(TEST_PLACE_ID, createPlaceReviewPage()) } returns createSlicePlaceReview()
                it("리뷰를 조회한다.") {
            ...
            context("유효한 placeId와 startId가 전달되면") {
                every { placeReviewRepository.getByPlaceIdStartId(TEST_PLACE_ID, TEST_START_ID, createPlaceReviewPage()) } returns createSlicePlaceReview()
                it("리뷰를 조회한다.") {
        ...
        describe("getByUserReviewList 메소드") {
            context("유효한 userId가 전달되면") {
                every { userRepository.getByUserId(TEST_USER_ID) } returns createUser()
                every { placeReviewRepository.getByUserInitial(any(), createPlaceReviewPage()) } returns createSlicePlaceReview()
                it("리뷰를 조회한다.") {
            ...
            context("유효한 userId와 startId가 전달되면") {
                every { userRepository.getByUserId(TEST_USER_ID) } returns createUser()
                every { placeReviewRepository.getByUserStartId(any(), TEST_START_ID, createPlaceReviewPage()) } returns createSlicePlaceReview()
                it("리뷰를 조회한다.") {
            ...
            context("존재하지 않는 userId가 전달되면") {
                every { userRepository.getByUserId(TEST_USER_ID) } throws NoSuchElementException()
                it("[NoSuchElementException] 예외가 발생한다") {
                    shouldThrow<NoSuchElementException> { sut.getByUserReviewList(TEST_USER_ID, TEST_START_ID, TEST_COUNT) }
                }
            }
        }
    }
