...
class CommunityControllerTest(
...
) : DescribeSpec(
    {
        ...
        describe("GET /api/v1/communities/like") {
            val targetUri = "/api/v1/communities/like"
            context("유효한 요청 데이터가 전달되면") {
                val response = createSliceCommunitySimpleResponse()
                every {
                ...
                it("200 응답한다.") {
                    restDocMockMvc.get(targetUri) {
                        ...
                        .andDo {
                            createDocument(
                                "community-get-liked-communities-success",
                                requestHeaders(
                                    HttpHeaders.AUTHORIZATION headerDescription "VALID ID TOKEN",
                                ),
                                queryParameters(
            ...
            context("유효하지 않은 토큰인 경우") {
                it("401 응답한다.") {
                    restDocMockMvc.get(targetUri) {
                        ...
                        .andDo {
                            createDocument(
                                "community-get-liked-communities-fail-invalid-token",
                                requestHeaders(
                                    HttpHeaders.AUTHORIZATION headerDescription "INVALID ID TOKEN",
                                ),
                                queryParameters(
                        ...
                        .andDo {
                            createDocument(
                                ...
                                responseBody(
                                    ...
                                    "content[].communityCommentSimpleResponse.user.isFollowing" type JsonFieldType.BOOLEAN description "댓글 작성자 팔로잉 여부" example response.content[0].communityCommentSimpleResponse.user.isFollowing,
